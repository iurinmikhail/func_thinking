...
from src.shipping_cart import add_product_to_cart, Product
...
class TestShippingCard(unittest.TestCase):
    ...
    def test_add_item_to_cart__when_paid_shipping(self, mock_logger: Mock, mock_cart: list) -> None:
    ...
    @patch("src.shipping_cart.SHOPPING_CART", new_callable=list)
    @patch("src.shipping_cart.logger.info")
    def test_add_item_to_cart__when_free_shipping(self, mock_logger: Mock, mock_cart: list) -> None:
